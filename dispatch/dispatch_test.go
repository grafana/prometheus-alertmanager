...
func TestDispatcher_DoMaintenance(t *testing.T) {
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, nil, promslog.NewNopLogger(), NewDispatcherMetrics(false, r), standardTimerFactory)
	...
	aggrGroup1 := newAggrGroup(ctx, labels, route, timeout, promslog.NewNopLogger(), standardTimerFactory)
