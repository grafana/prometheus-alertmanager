...
type NotificationLog interface {
	Log(r *nflogpb.Receiver, gkey string, firingAlerts, resolvedAlerts []uint64, repeatInterval time.Duration) error
...
func (n SetNotifiesStage) Exec(ctx context.Context, l log.Logger, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
	...
	return ctx, alerts, n.nflog.Log(n.recv, gkey, firing, resolved, repeat)
}
