...
func (n *DedupStage) Exec(ctx context.Context, l log.Logger, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
	...
	_ = level.Warn(l).Log("msg", "needsUpdate", "firingSet", firingSet, "resolvedSet", repeatInterval, "needsUpdate", needsUpdate, "needsUpdateReason", reason, "receiver", n.recv.GroupName, "integration", n.recv.Integration)
	if !needsUpdate {
		return ctx, nil, nil
	}
