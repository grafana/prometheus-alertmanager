...
func TestWebex_PreSendHook(t *testing.T) {
	...
	notifierWebex, err := WithCustomMarshall(
		func(ctx context.Context, payload interface{}, alerts []*types.Alert) (*bytes.Buffer, error) {
			...
			case webhook:
				...
				return &buffer, nil
			default:
				return nil, fmt.Errorf("cannot recognize payload of type %T, expected webhook", p)
			}
		})(New(cfg, test.CreateTmpl(t), log.NewNopLogger()))
	require.NoError(t, err)
